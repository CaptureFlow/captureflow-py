...
class Orchestrator:
    ...
    def find_improvement_candidates(self, graphs: List[CallGraph], top_n: int = 5) -> Dict[str, float]:
        ...
        for candidate, data in preselected_candidates.items():
            ...
            if gpt_response_dict.get("easy_to_optimize") == "EASY_TO_OPTIMIZE":
                ...
                candidate_scores[candidate] = gpt_response_dict.get("quality_score", 0)  # Default to 0 if not available
    ...
    def improve_and_validate(self, graphs: List[CallGraph], function_name: str) -> Tuple[Dict[str, int], Optional[str], Optional[Any]]:
...
def validate_graphs(graphs: List[CallGraph], new_implementation: str, function_name: str, gpt_helper: OpenAIHelper) -> Dict[str, int]:
...
if __name__ == '__main__':
    ...
    sample_trace_data = load_sample_trace("/Users/nikitakutc/projects/captureflow-py/serverside/tests/assets/sample_trace.json")
