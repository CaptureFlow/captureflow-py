...
class RepoHelper:
        ...
        try:
            ...
            for symbol_type, node in visitor.definitions:
                ...
                index[symbol_type][symbol_name].append({
                    ...
                    "content": "\n".join(file_data.splitlines()[node.lineno-1:node.end_lineno if hasattr(node, 'end_lineno') else node.lineno])
                })
    ...
    def create_pull_request_with_new_function(self, node, new_implementation: str, gpt_helper: OpenAIHelper):
        ...
        new_code_lines = source_code_lines[:start_line-1] + new_implementation.splitlines() + source_code_lines[end_line:]
        updated_source_code = "\n".join(new_code_lines)
        ...
        self.gh_repo.update_file(file_path, commit_message, updated_source_code, content.sha, branch=new_branch_name)
